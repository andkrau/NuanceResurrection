...
static void context_reset(void)
{
    ...
    if (glewInit() != GLEW_OK)
        log_printf("libretro: glewInit failed\n");
    else
        log_printf("libretro: GL context reset OK\n");
...
static std::string FindGameFile(const char* path)
{
    ...
    char tmpl[] = "/tmp/nuance_lr_XXXXXX";
    ...
    // Extract NUON directory from ISO
...
bool retro_load_game(const struct retro_game_info *game)
{
    ...
    // Find bios files - check system directory
...
void retro_run(void)
{
    ...
    // Run emulation until next video frame
    nuonEnv.trigger_render_video = false;
    uint32 cycles = 0;
    while (bRun && !nuonEnv.trigger_render_video && cycles < 500000)
    {
        cycles++;
        ...
        if ((cycles % 500) == 0)
        {
            ...
            if (nuonEnv.timer_rate[2] > 0 && new_time >= last_time2 + (uint64)nuonEnv.timer_rate[2]) {
                ...
                last_time2 = new_time;
            } else if (nuonEnv.timer_rate[2] <= 0) last_time2 = new_time;
        }
