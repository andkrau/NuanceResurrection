...
void Emit_DECRc0(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  //rc0M1 = rc0 - 1
  vars->mpe->nativeCodeCache.X86Emit_DECR(x86Reg_ebx);
  //if(rc0M1 >= 0), rc0WriteBack = rc0 (otherwise rc0M1)
  vars->mpe->nativeCodeCache.X86Emit_CMOVNLRR(x86Reg_eax, x86Reg_ebx);
...
void Emit_RangeOnly(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_CMPRR(x86Reg_ecx, x86Reg_eax);
  vars->mpe->nativeCodeCache.X86Emit_JCC_Label(vars->mpe->nativeCodeCache.patchMgr,X86_CC_L,l_testmodmi);

  vars->mpe->nativeCodeCache.X86Emit_SUBRR(x86Reg_ebx, x86Reg_eax);
  ...
  vars->mpe->nativeCodeCache.X86Emit_CMPIR(0,x86Reg_ebx);
