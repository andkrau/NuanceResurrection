...
{
  const uint64 src1 = (uint64)entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC1]];
  const uint64 src2 = (uint64)entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC2]];
...
void Execute_COPY(MPE &mpe, const InstructionCacheEntry &entry, const Nuance &nuance)
{
  ...
  const uint32 src = entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC2]];
...
void Execute_SAT(MPE &mpe, const InstructionCacheEntry &entry, const Nuance &nuance)
{
  ...
  const int32 n = entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC1]];
  const uint32 dest = nuance.fields[FIELD_ALU_DEST];
...
{
        uint32 src1 = entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC1]] & 0x3FUL;
  const uint32 src2 = entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC2]];
...
{
  const uint64 src1 = (((int32)entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC1]]) >>
...
{
  const uint64 src1 = (uint64)(((int32)entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC1]]) >>
    nuance.fields[FIELD_ALU_SRC2]);
  const uint64 src2 = (uint64)entry.pScalarRegs[nuance.fields[FIELD_ALU_DEST]];
...
{
  mpe.cc &= ~(CC_ALU_ZERO | CC_ALU_NEGATIVE | CC_ALU_OVERFLOW);
  const uint32 src1 = entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC1]];
  const uint32 src2 = entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC2]];
...
void Execute_ANDImmediateShiftScalar(MPE &mpe, const InstructionCacheEntry &entry, const Nuance &nuance)
{
  ...
  const uint32 src1 = nuance.fields[FIELD_ALU_SRC1];
  const uint32 src2 = (((int32)(entry.pScalarRegs[nuance.fields[FIELD_ALU_SRC2]] << 26)) >> 26) & 0x3FUL;
        uint32 dest = entry.pScalarRegs[nuance.fields[FIELD_ALU_DEST]];
