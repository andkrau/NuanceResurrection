...
bool SuperBlock::EmitCodeBlock(NativeCodeCache &codeCache, const bool bContainsBranch)
{
  ...
  else if(compileType == SuperBlockCompileType::SUPERBLOCKCOMPILETYPE_NATIVE_CODE_BLOCK)
  {
    ...
    codeCache.X86Emit_MOVIM(exitAddress, x86MemPtr::x86MemPtr_dword,(uintptr_t)&(codeCache.emitVars.mpe->pcexec));
...
#ifdef USE_ASMJIT
    const uint32 emittedBytes = codeCache.AsmJit_EndBlock();
#else
