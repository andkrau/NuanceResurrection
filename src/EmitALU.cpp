...
{
  const uint32 destRegIndex = (uint32_t)nuance.fields[FIELD_ALU_DEST];
  const uint32 src1RegIndex = (uint32_t)nuance.fields[FIELD_ALU_SRC1];
...
{
  const uint32 destRegIndex = (uint32_t)nuance.fields[FIELD_ALU_DEST];
  const uint32 src1Imm = (uint32_t)nuance.fields[FIELD_ALU_SRC1];
  const uint32 src2RegIndex = (uint32_t)nuance.fields[FIELD_ALU_SRC2];
...
void Emit_BITSImmediate(EmitterVariables * const vars, const Nuance &nuance)
{
  const uint32 destRegIndex = (uint32_t)nuance.fields[FIELD_ALU_DEST];
  const uint32 src1Imm = (uint32_t)nuance.fields[FIELD_ALU_SRC1];
  const uint32 src2Imm = (uint32_t)nuance.fields[FIELD_ALU_SRC2];
  const x86BaseReg destRegReadBaseReg = GetScalarRegReadBaseReg(vars,destRegIndex);
  const x86BaseReg destRegWriteBaseReg = GetScalarRegWriteBaseReg(vars,destRegIndex);
  //const x86BaseReg ccRegReadBaseReg = GetMiscRegReadBaseReg(vars,REGINDEX_CC);
  const x86BaseReg ccRegWriteBaseReg = GetMiscRegWriteBaseReg(vars,REGINDEX_CC);
...
void Emit_BTST(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  const uint32 mask = (uint32_t)nuance.fields[FIELD_ALU_SRC1];
  const uint32 src2RegIndex = (uint32_t)nuance.fields[FIELD_ALU_SRC2];
  const x86BaseReg src2RegReadBaseReg = GetScalarRegReadBaseReg(vars,src2RegIndex);
  //const x86BaseReg ccRegReadBaseReg = GetMiscRegReadBaseReg(vars,REGINDEX_CC);
  const x86BaseReg ccRegWriteBaseReg = GetMiscRegWriteBaseReg(vars,REGINDEX_CC);
...
void Emit_BUTT(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  if(vars->scalarRegOutDep & (1u << (destRegIndex+1)))
  {
  ...
  if(vars->scalarRegOutDep & (1u << destRegIndex))
  {
    vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_ebx, destRegWriteBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, destRegDisp);
  }
...
void Emit_MSB(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  //n = (n ^ (n >> 31)) - folds {0, -1} -> 0 and negatives to ~n, so BSR's ZF flags the sigbits==0 case
  vars->mpe->nativeCodeCache.X86Emit_MOVRR(x86Reg::x86Reg_ebx, x86Reg::x86Reg_eax);
  vars->mpe->nativeCodeCache.X86Emit_SARIR(x86Reg::x86Reg_ebx, 31);
  vars->mpe->nativeCodeCache.X86Emit_XORRR(x86Reg::x86Reg_eax, x86Reg::x86Reg_ebx);

  //bsr ebx, eax -> ebx = index of most significant set bit; ZF=1 (ebx undefined) if eax==0
  vars->mpe->nativeCodeCache.X86Emit_BSRRR(x86Reg::x86Reg_ebx, x86Reg::x86Reg_eax);

  //force ebx to -1 when source was zero so the following INC yields sigbits == 0
  vars->mpe->nativeCodeCache.X86Emit_MOVIR(-1, x86Reg::x86Reg_ecx);
  vars->mpe->nativeCodeCache.X86Emit_CMOVZRR(x86Reg::x86Reg_ebx, x86Reg::x86Reg_ecx);

  //sigbits = bsr_index + 1
  vars->mpe->nativeCodeCache.X86Emit_INCR(x86Reg::x86Reg_ebx);
...
void Emit_ADD_SV(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  {
    vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm2, (uintptr_t)&sub_sv_mask);
    ...
    vars->mpe->nativeCodeCache.X86Emit_PANDRR(x86Reg::x86Reg_xmm1, x86Reg::x86Reg_xmm2);
    vars->mpe->nativeCodeCache.X86Emit_PADDRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  ...
  if(src2RegIndex != src1RegIndex)
  {
    if( ((src1RegReadBaseReg_0 == src1RegReadBaseReg_1) && (src1RegReadBaseReg_2 == src1RegReadBaseReg_3)) &&
        ((src2RegReadBaseReg_0 == src2RegReadBaseReg_1) && (src2RegReadBaseReg_2 == src2RegReadBaseReg_3)) )
      {
        ...
        vars->mpe->nativeCodeCache.X86Emit_PANDRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm2);
        vars->mpe->nativeCodeCache.X86Emit_PANDRR(x86Reg::x86Reg_xmm1, x86Reg::x86Reg_xmm2);
        vars->mpe->nativeCodeCache.X86Emit_PSUBDRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
        vars->mpe->nativeCodeCache.X86Emit_MOVDQURM(x86Reg::x86Reg_xmm0, destRegWriteBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, destRegDisp);
      }
      else
      {
...
void Emit_ASR(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_ebx, src2RegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, src2RegDisp);
  // Carry flag needs bit 0 of original src2 routed to CC_ALU_CARRY (bit 1) position.
  // Fuse "save copy + SHL by 1" into one LEA, and gate on DEP_C so it's a no-op when the carry isn't consumed later-on
  if(vars->miscRegOutDep & DEPENDENCY_FLAG_C)
  {
    vars->mpe->nativeCodeCache.X86Emit_LEA(x86Reg::x86Reg_ecx, (uintptr_t)x86BaseReg::x86BaseReg_ebx, x86IndexReg::x86IndexReg_ebx, x86ScaleVal::x86Scale_1, 0);
  }
  if(src1Imm)
  {
  ...
  if(vars->miscRegOutDep & DEPENDENCY_FLAG_NVCZ)
  {
    ...
    if(vars->miscRegOutDep & DEPENDENCY_FLAG_C)
    {
      // ecx already holds 2*src2 from the LEA above; just mask to extract the bit
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(CC_ALU_CARRY, x86Reg::x86Reg_ecx);
    }
