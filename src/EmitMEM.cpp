...
// EBX contains bank_index * 4 (from shr 26; and 0x3C).
// On 32-bit: mov ebx, [bankPtrTable + ebx*1] (4-byte pointer entries)
...
static inline void Emit_LoadBankPtr(EmitterVariables * const vars)
{
  ...
  if (cc.asmjitAs) {
    ...
    a.mov(asmjit::x86::r15, (uint64_t)(uintptr_t)vars->mpe->bankPtrTable);
    // Load 64-bit pointer: bankPtrTable[index] where index = ebx*4, scale=2 -> byte offset = ebx*4*2 = ebx*8
...
static const __m128i bswap_short_load_lut = _mm_set_epi8(6, 7, (char)0x80, (char)0x80, 4, 5, (char)0x80, (char)0x80, 2, 3, (char)0x80, (char)0x80, 0, 1, (char)0x80, (char)0x80);
static const __m128i bswap_short_store_lut = _mm_set_epi8((char)0x80, (char)0x80, (char)0x80, (char)0x80, (char)0x80, (char)0x80, (char)0x80, (char)0x80, 14, 15, 10, 11, 6, 7, 2, 3);
...
void Emit_Mirror(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  if(vars->scalarRegOutDep)
  {
    ...
    // Swap adjacent bits - ((a>>1) & 0x55..) | ((a<<1) & 0xAA..)
    vars->mpe->nativeCodeCache.X86Emit_LEA(x86Reg::x86Reg_ebx, (uintptr_t)x86BaseReg::x86BaseReg_eax, x86IndexReg::x86IndexReg_eax, x86ScaleVal::x86Scale_1, 0);
    vars->mpe->nativeCodeCache.X86Emit_SHRIR(x86Reg::x86Reg_eax, 1);
    vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x55555555, x86Reg::x86Reg_eax);
    vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xAAAAAAAA, x86Reg::x86Reg_ebx);
    vars->mpe->nativeCodeCache.X86Emit_ORRR(x86Reg::x86Reg_eax, x86Reg::x86Reg_ebx);

    // Swap adjacent 2-bit pairs - ((a>>2) & 0x33..) | ((a<<2) & 0xCC..)
    vars->mpe->nativeCodeCache.X86Emit_LEA(x86Reg::x86Reg_ebx, X86_NO_BASE, x86IndexReg::x86IndexReg_eax, x86ScaleVal::x86Scale_4, 0);
    ...
    vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xCCCCCCCC, x86Reg::x86Reg_ebx);
    vars->mpe->nativeCodeCache.X86Emit_ORRR(x86Reg::x86Reg_eax, x86Reg::x86Reg_ebx);
    ...
    // After the three bit-reversal stages above, eax = [B3-rev, B2-rev, B1-rev, B0-rev].
    // BSWAP reverses byte order to [B0-rev, B1-rev, B2-rev, B3-rev]
...
void Emit_LoadByteBilinearXY(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_CALLI((uintptr_t)GetBilinearAddress,0);
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_edx, (uintptr_t)&vars->mpe->xybase);
...
void Emit_LoadByteBilinearUV(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  // returns result in eax
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x7FFFFFFFu, x86Reg::x86Reg_eax); // clear the passed back 4bit pixel addressing bit in MSB
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_edx, (uintptr_t)&vars->mpe->uvbase);
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFCu, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_ADDRR(x86Reg::x86Reg_eax, x86Reg::x86Reg_edx);
...
void Emit_LoadShortVectorAbsolute(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  if(SSSE3_supported)
  {
  vars->mpe->nativeCodeCache.X86Emit_MOVQMR(x86Reg::x86Reg_xmm0, srcAddress);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm1, (uintptr_t)&bswap_short_load_lut);
...
void Emit_LoadShortVectorLinear(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_ADDRR64(x86Reg::x86Reg_eax, x86Reg::x86Reg_ebx);
...
void Emit_LoadVectorAbsolute(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm1, (uintptr_t)&bswap_lut);
  vars->mpe->nativeCodeCache.X86Emit_PSHUFBRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQURM(x86Reg::x86Reg_xmm0, destRegWriteBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, destRegDisp);
  }
  else
...
void Emit_LoadPixelAbsolute(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_eax, (uintptr_t)&vars->mpe->linpixctl);
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(destRegIndex, x86MemPtr::x86MemPtr_dword, (uintptr_t)&vars->mpe->ba_reg_offset);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_eax, (uintptr_t)&vars->mpe->ba_control);

  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)vars->mpe, x86Reg::x86Reg_ecx);
  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)nuance.fields[FIELD_MEM_POINTER], x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_CALLI((uintptr_t)_LoadPixelAbsolute,0);
}
...
void Emit_LoadPixelZAbsolute(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)vars->mpe, x86Reg::x86Reg_ecx);
  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)nuance.fields[FIELD_MEM_POINTER], x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_CALLI((uintptr_t)_LoadPixelZAbsolute,0);
}
...
void Emit_LoadPixelLinear(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_ecx, (uintptr_t)&vars->mpe->linpixctl);
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(destRegIndex, x86MemPtr::x86MemPtr_dword, (uintptr_t)&vars->mpe->ba_reg_offset);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_ecx, (uintptr_t)&vars->mpe->ba_control);
  ...
  vars->mpe->nativeCodeCache.X86Emit_ADDRR64(x86Reg::x86Reg_edx, x86Reg::x86Reg_ebx); // edx passed in below
...
void Emit_LoadPixelBilinearUV(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_SHRIR(x86Reg::x86Reg_ebx,16);
  vars->mpe->nativeCodeCache.X86Emit_ORRR(x86Reg::x86Reg_ecx, x86Reg::x86Reg_ebx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_edx, (uintptr_t)&vars->mpe->ba_control); // needed by _LoadPixelAbsolute
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(destRegIndex, x86MemPtr::x86MemPtr_dword, (uintptr_t)&vars->mpe->ba_reg_offset);

  vars->mpe->nativeCodeCache.X86Emit_CALLI((uintptr_t)GetBilinearAddress,0);
  // returns result in eax
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_eax, (uintptr_t)&vars->mpe->ba_mipped_xoffset); // store the 4bit pixel addressing bit
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x7FFFFFFFu, x86Reg::x86Reg_eax); // clear the passed back 4bit pixel addressing bit in MSB
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_edx, (uintptr_t)&vars->mpe->uvbase);
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFCu, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_ADDRR(x86Reg::x86Reg_eax, x86Reg::x86Reg_edx);
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)vars->mpe, x86Reg::x86Reg_ecx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRR64(x86Reg::x86Reg_edx, x86Reg::x86Reg_eax);
  vars->mpe->nativeCodeCache.X86Emit_CALLI((uintptr_t)_LoadPixelAbsolute,0);
}
...
void Emit_LoadPixelZBilinearUV(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_SHRIR(x86Reg::x86Reg_ebx,16);
  vars->mpe->nativeCodeCache.X86Emit_ORRR(x86Reg::x86Reg_ecx, x86Reg::x86Reg_ebx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_edx, (uintptr_t)&vars->mpe->ba_control); // needed by _LoadPixelZAbsolute
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(destRegIndex, x86MemPtr::x86MemPtr_dword, (uintptr_t)&vars->mpe->ba_reg_offset);
...
{
  constexpr uint32 l_not_special = 0;
  ...
  // Bail to interpreter for any address in [MPE_ITAGS_BASE, MPE_RESV_BASE):
  // the ITAGS subrange triggers icache invalidation and the CTRL subrange writes control registers.
  // Done via a single unsigned-range check: (address - ITAGS_BASE) < (RESV_BASE - ITAGS_BASE).
  // Wraps negative for addresses below ITAGS_BASE, so JCC_NB takes the normal-store path
  // for both below-range and at-or-above-range addresses
  vars->mpe->nativeCodeCache.X86Emit_MOVRR(x86Reg::x86Reg_ebx, x86Reg::x86Reg_eax);
  vars->mpe->nativeCodeCache.X86Emit_SUBIR(MPE_ITAGS_BASE, x86Reg::x86Reg_ebx);
  vars->mpe->nativeCodeCache.X86Emit_CMPIR(MPE_RESV_BASE - MPE_ITAGS_BASE, x86Reg::x86Reg_ebx);
  vars->mpe->nativeCodeCache.X86Emit_JCC_Label(X86_CC_NB, l_not_special);
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(1, x86MemPtr::x86MemPtr_dword, (uintptr_t)&(vars->mpe->interpretNextPacket));
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(vars->pInstructionEntry->packet->pcexec, x86MemPtr::x86MemPtr_dword, (uintptr_t)&(vars->mpe->pcexec));
  Emit_ExitBlock(vars);
  vars->mpe->nativeCodeCache.SetLabelPointer(l_not_special);
  vars->mpe->nativeCodeCache.X86Emit_MOVRR(x86Reg::x86Reg_ebx, x86Reg::x86Reg_eax);
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFF00000, x86Reg::x86Reg_ebx);
  vars->mpe->nativeCodeCache.X86Emit_SHRIR(x86Reg::x86Reg_ebx,26);
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x3C,x86Reg::x86Reg_ebx);
...
void Emit_StoreShortVectorAbsolute(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm1, (uintptr_t)&bswap_short_store_lut);
  vars->mpe->nativeCodeCache.X86Emit_PSHUFBRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVQRM(x86Reg::x86Reg_xmm0, destAddress);
  }
  else
  {
...
void Emit_StorePixelAbsolute(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_ecx, (uintptr_t)&vars->mpe->linpixctl);
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(srcRegIndex, x86MemPtr::x86MemPtr_dword, (uintptr_t)&vars->mpe->ba_reg_offset);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_ecx, (uintptr_t)&vars->mpe->ba_control);

  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)vars->mpe, x86Reg::x86Reg_ecx);
  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)nuance.fields[FIELD_MEM_POINTER], x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_CALLI((uintptr_t)_StorePixelAbsolute,0);
}
...
void Emit_StorePixelZAbsolute(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)vars->mpe, x86Reg::x86Reg_ecx);
  vars->mpe->nativeCodeCache.X86Emit_MOVIR_Ptr((uintptr_t)nuance.fields[FIELD_MEM_POINTER], x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_CALLI((uintptr_t)_StorePixelZAbsolute,0);
}
...
void Emit_StorePixelBilinearUV(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_SHRIR(x86Reg::x86Reg_ebx,16);
  vars->mpe->nativeCodeCache.X86Emit_ORRR(x86Reg::x86Reg_ecx, x86Reg::x86Reg_ebx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_edx, (uintptr_t)&vars->mpe->ba_control); // needed by _StorePixelAbsolute
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(srcRegIndex, x86MemPtr::x86MemPtr_dword, (uintptr_t)&vars->mpe->ba_reg_offset);
...
void Emit_StorePixelZBilinearUV(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_SHRIR(x86Reg::x86Reg_ebx,16);
  vars->mpe->nativeCodeCache.X86Emit_ORRR(x86Reg::x86Reg_ecx, x86Reg::x86Reg_ebx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_edx, (uintptr_t)&vars->mpe->ba_control); // needed by _StorePixelZAbsolute
  vars->mpe->nativeCodeCache.X86Emit_MOVIM(srcRegIndex, x86MemPtr::x86MemPtr_dword, (uintptr_t)&vars->mpe->ba_reg_offset);
...
void Emit_PushVector(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_edx, (uintptr_t)&(vars->mpe->sp));

  if (SSSE3_supported && (srcRegReadBaseReg_0 == srcRegReadBaseReg_1) && (srcRegReadBaseReg_2 == srcRegReadBaseReg_3) && (srcRegReadBaseReg_0 == srcRegReadBaseReg_2))
  {
  vars->mpe->nativeCodeCache.X86Emit_MOVDQUMR(x86Reg::x86Reg_xmm0, srcRegReadBaseReg_0, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, srcRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_SUBIR(16, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_edx, (uintptr_t)&(vars->mpe->sp));
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(MPE_VALID_MEMORY_MASK, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_ADDIR((uintptr_t)vars->mpe->dtrom, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm1, (uintptr_t)&bswap_lut);
  vars->mpe->nativeCodeCache.X86Emit_PSHUFBRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQURM(x86Reg::x86Reg_xmm0, x86BaseReg::x86BaseReg_edx, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, 0);
  }
  else
  {
  vars->mpe->nativeCodeCache.X86Emit_MOVMR(x86Reg::x86Reg_eax, srcRegReadBaseReg_0, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, srcRegDisp);
...
void Emit_PushVectorRz(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  if (SSSE3_supported && (srcRegReadBaseReg_0 == srcRegReadBaseReg_1) && (srcRegReadBaseReg_0 == srcRegReadBaseReg_2))
  {
  vars->mpe->nativeCodeCache.X86Emit_MOVQMR(x86Reg::x86Reg_xmm0, srcRegReadBaseReg_0, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, srcRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm1, srcRegReadBaseReg_0, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, srcRegDisp+8);
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm2, rzRegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, rzRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_UNPCKLRR(x86Reg::x86Reg_xmm1, x86Reg::x86Reg_xmm2);
  vars->mpe->nativeCodeCache.X86Emit_MOVLHRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_SUBIR(16, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_edx, (uintptr_t)&(vars->mpe->sp));
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(MPE_VALID_MEMORY_MASK, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_ADDIR((uintptr_t)vars->mpe->dtrom, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm3, (uintptr_t)&bswap_lut);
  vars->mpe->nativeCodeCache.X86Emit_PSHUFBRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm3);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQURM(x86Reg::x86Reg_xmm0, x86BaseReg::x86BaseReg_edx, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, 0);
  }
  else
  {
...
void Emit_PushScalarRzi1(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  if (SSSE3_supported)
  {
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm0, srcRegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, srcRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm1, ccRegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, ccRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_UNPCKLRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm2, rzi1RegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, rzi1RegDisp);
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm1, rzRegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, rzRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_UNPCKLRR(x86Reg::x86Reg_xmm2, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVLHRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm2);
  vars->mpe->nativeCodeCache.X86Emit_SUBIR(16, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_MOVRM(x86Reg::x86Reg_edx, (uintptr_t)&(vars->mpe->sp));
  vars->mpe->nativeCodeCache.X86Emit_ANDIR(MPE_VALID_MEMORY_MASK, x86Reg::x86Reg_edx);
  vars->mpe->nativeCodeCache.X86Emit_ADDIR((uintptr_t)vars->mpe->dtrom, x86Reg::x86Reg_edx);
...
void Emit_PushScalarRzi2(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm1, ccRegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, ccRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_UNPCKLRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm2, rzi2RegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, rzi2RegDisp);
  vars->mpe->nativeCodeCache.X86Emit_MOVSSMR(x86Reg::x86Reg_xmm1, rzRegReadBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, rzRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_UNPCKLRR(x86Reg::x86Reg_xmm2, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVLHRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm2);
  vars->mpe->nativeCodeCache.X86Emit_SUBIR(16, x86Reg::x86Reg_edx);
...
void Emit_PopVector(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_ADDIR((uintptr_t)vars->mpe->dtrom, x86Reg::x86Reg_ebp);

  if (SSSE3_supported)
  {
  vars->mpe->nativeCodeCache.X86Emit_MOVDQUMR(x86Reg::x86Reg_xmm0, x86BaseReg::x86BaseReg_ebp, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, 0);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm1, (uintptr_t)&bswap_lut);
  vars->mpe->nativeCodeCache.X86Emit_PSHUFBRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQURM(x86Reg::x86Reg_xmm0, destRegWriteBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, destRegDisp);
  }
...
void Emit_PopVectorRz(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  if (SSSE3_supported)
  {
  vars->mpe->nativeCodeCache.X86Emit_MOVDQUMR(x86Reg::x86Reg_xmm0, x86BaseReg::x86BaseReg_edx, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, 0);
  vars->mpe->nativeCodeCache.X86Emit_MOVDQAMR(x86Reg::x86Reg_xmm1, (uintptr_t)&bswap_lut);
  vars->mpe->nativeCodeCache.X86Emit_PSHUFBRR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm1);

  vars->mpe->nativeCodeCache.X86Emit_MOVQRM(x86Reg::x86Reg_xmm0, destRegWriteBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, destRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_SHUFIR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm0, 0x0E); // xmm0 = [v2, v3, v0, v0]
...
void Emit_PopScalarRzi1(EmitterVariables * const vars, const Nuance &nuance)
{
  ...
  vars->mpe->nativeCodeCache.X86Emit_MOVDRM(x86Reg::x86Reg_xmm0, destRegWriteBaseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, destRegDisp);
  vars->mpe->nativeCodeCache.X86Emit_SHUFIR(x86Reg::x86Reg_xmm0, x86Reg::x86Reg_xmm0, 0x39); // [v1, v2, v3, v0]
...
void EmitControlRegisterLoad(EmitterVariables * const vars, const uint32 address, const x86Reg destReg)
{
  switch((address & 0x1FF0) >> 4)
  {
    case (0x00 >> 4):
      //mpectl
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->mpectl)));
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(~(0xF0FF5555U), destReg);
      break;
    case (0x10 >> 4):
      //excepsrc
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->excepsrc)));
      break;
    case (0x20 >> 4):
    ...
    case (0x30 >> 4):
      //excephalten
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->excephalten)));
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x1FFFU, destReg);
      break;
    case (0x40 >> 4):
      //cc
      {
      ...
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x7FFU, destReg);
      }
      break;
    case (0x80 >> 4):
      //rz
    ...
    case (0xB0 >> 4):
      //intvec1
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->intvec1)));
      break;
    case (0xC0 >> 4):
      //intvec2
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->intvec2)));
      break;
    case (0xD0 >> 4):
      //intsrc
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->intsrc)));
      break;
    case (0xE0 >> 4):
    ...
    case (0xF0 >> 4):
      //intctl
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->intctl)));
    ...
    case (0x110 >> 4):
      ...
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->inten1)));
      break;
    case (0x120 >> 4):
    ...
    case (0x130 >> 4):
      //inten2sel
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->inten2sel)));
    ...
    case (0x220 >> 4):
      ...
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, baseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, disp);
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(((0x3FFU << 16) | 0x3FFU), destReg);
      }
      break;
    case (0x230 >> 4):
      //xybase
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->xybase)));
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFCU, destReg);
      break;
    case (0x240 >> 4):
      //xyctl
      {
      ...
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, baseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, disp);
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(~((1U << 11) | (1U << 27) | (1U << 31)), destReg);
      }
      break;
    case (0x250 >> 4):
      //ru
    ...
    case (0x280 >> 4):
      //uvbase
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->uvbase)));
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFCU, destReg);
      break;
    case (0x290 >> 4):
      //uvctl
    ...
    case (0x2A0 >> 4):
      //linpixctl
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->linpixctl)));
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(((1U << 28) | (15U << 20)), destReg);
      break;
    case (0x2B0 >> 4):
      //clutbase
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->clutbase)));
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFC0U, destReg);
      break;
    case (0x2C0 >> 4):
      //svshift
      {
      ...
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, baseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, disp);
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x03U, destReg);
      }
      break;
    case (0x2D0 >> 4):
      //acshift
    ...
    case (0x2E0 >> 4):
      //sp
      vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->sp)));
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFF0U, destReg);
      break;
    case (0x500 >> 4):
      //odmactl
    ...
    case (0xFF0 >> 4):
      //configa/configb/dcachectl/icachectl
      switch(address & 0x0F)
      {
        case 0x00:
          vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->configa)));
          break;
        case 0x04:
          vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->configb)));
          break;
        case 0x08:
          vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->dcachectl)));
          break;
        case 0x0C:
          vars->mpe->nativeCodeCache.X86Emit_MOVMR(destReg, ((uintptr_t)&(vars->mpe->icachectl)));
          break;
        default:
          vars->mpe->nativeCodeCache.X86Emit_XORRR(destReg, destReg);
          break;
...
void EmitControlRegisterStore(EmitterVariables * const vars, const uint32 address, const x86Reg srcReg)
{
  ...
  switch((address & 0x1FF0) >> 4)
  {
    ...
    case (0x20 >> 4):
      ...
      vars->mpe->nativeCodeCache.X86Emit_ANDRM(srcReg,((uintptr_t)&(vars->mpe->excepsrc)));
      break;
    //case (0x30 >> 4):
      //excephalten
    //  break;
    case (0x40 >> 4):
      //cc
      {
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0x7FFU, srcReg);
    ...
    case (0xB0 >> 4):
      //intvec1
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFEU, srcReg);
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, ((uintptr_t)&(vars->mpe->intvec1)));
      break;
    case (0xC0 >> 4):
      //intvec2
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFEU, srcReg);
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, ((uintptr_t)&(vars->mpe->intvec2)));
      break;
    //case (0xD0 >> 4):
      //intsrc
    //  break;
    case (0xE0 >> 4):
      ...
      vars->mpe->nativeCodeCache.X86Emit_NOTR(srcReg);
      vars->mpe->nativeCodeCache.X86Emit_ANDRM(srcReg, ((uintptr_t)&(vars->mpe->intsrc)));
      break;
    //case (0xF0 >> 4):
      //intctl
    //  break;
    ...
    case (0x230 >> 4):
      ...
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFCU, srcReg);
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, ((uintptr_t)&(vars->mpe->xybase)));
      break;
    case (0x240 >> 4):
      //xyctl
      {
      ...
      const int32 disp = GetMiscRegEmitDisp(vars,REGINDEX_XYCTL);
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(~((1U << 31) | (1U << 27) | (1U << 11)), srcReg);
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, baseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, disp);
      }
      break;
    case (0x250 >> 4):
    ...
    case (0x280 >> 4):
      //uvbase
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFFCU, srcReg);
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, ((uintptr_t)&(vars->mpe->uvbase)));
      break;
    case (0x290 >> 4):
      //uvctl
      {
    ...
    case (0x2A0 >> 4):
      //linpixctl
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(((0x01U << 28) | (0xFU << 20)), srcReg);
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, ((uintptr_t)&(vars->mpe->linpixctl)));
      break;
    case (0x2B0 >> 4):
      //clutbase
      vars->mpe->nativeCodeCache.X86Emit_ANDIR(0xFFFFFFC0U, srcReg);
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, ((uintptr_t)&(vars->mpe->clutbase)));
      break;
    case (0x2C0 >> 4):
      // svshift
      {
    ...
    case (0x2E0 >> 4):
      ...
      vars->mpe->nativeCodeCache.X86Emit_MOVRM(srcReg, ((uintptr_t)&(vars->mpe->sp)));
      break;
    case (0x500 >> 4):
      //odmactl
    case (0x510 >> 4):
...
void EmitControlRegisterStoreImmediate(EmitterVariables * const vars, const uint32 address, const uint32 imm)
{
  ...
  switch((address & 0x1FF0) >> 4)
  {
    ...
    //  break;
    case (0x20 >> 4):
      vars->mpe->nativeCodeCache.X86Emit_ANDIM(~imm, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->excepsrc)));
      break;
    //case (0x30 >> 4):
      //excephalten
    //  break;
    case (0x40 >> 4):
      ...
      const x86BaseReg baseReg = GetMiscRegWriteBaseReg(vars,REGINDEX_CC);
      const int32 disp = GetMiscRegEmitDisp(vars,REGINDEX_CC);
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0x7FFU, x86MemPtr::x86MemPtr_dword, baseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, disp);
      }
      break;
    case (0x80 >> 4):
      //rz: 16-bit instruction-address alignment
    ...
    case (0xB0 >> 4):
      //intvec1
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0xFFFFFFFEU, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->intvec1)));
      break;
    case (0xC0 >> 4):
      //intvec2
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0xFFFFFFFEU, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->intvec2)));
      break;
    //case (0xD0 >> 4):
      //intsrc
    //  break;
    case (0xE0 >> 4):
      //intclr
      vars->mpe->nativeCodeCache.X86Emit_ANDIM(~imm, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->intsrc)));
      break;
    //case (0xF0 >> 4):
      //intctl
    //  break;
    ...
    case (0x230 >> 4):
      //xybase
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0xFFFFFFFCU, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->xybase)));
      break;
    case (0x240 >> 4):
      //xyctl
      {
      const x86BaseReg baseReg = GetMiscRegWriteBaseReg(vars,REGINDEX_XYCTL);
      const int32 disp = GetMiscRegEmitDisp(vars,REGINDEX_XYCTL);
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & (~((1U << 31) | (1U << 27) | (1U << 11))), x86MemPtr::x86MemPtr_dword, baseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, disp);
      }
      break;
    case (0x250 >> 4):
      //ru
    ...
    case (0x280 >> 4):
      //uvbase
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0xFFFFFFFCU, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->uvbase)));
      break;
    case (0x290 >> 4):
      //uvctl
      {
    ...
    case (0x2A0 >> 4):
      //linpixctl
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & ((0x01U << 28) | (0xFU << 20)), x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->linpixctl)));
      break;
    case (0x2B0 >> 4):
      //clutbase
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0xFFFFFFC0U, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->clutbase)));
      break;
    case (0x2C0 >> 4):
      //svshift
      {
      const x86BaseReg baseReg = GetMiscRegWriteBaseReg(vars,REGINDEX_SVSHIFT);
      const int32 disp = GetMiscRegEmitDisp(vars,REGINDEX_SVSHIFT);
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0x03U, x86MemPtr::x86MemPtr_dword, baseReg, x86IndexReg::x86IndexReg_none, x86ScaleVal::x86Scale_1, disp);
      }
      break;
    case (0x2D0 >> 4):
      //acshift
    ...
    case (0x2E0 >> 4):
      //sp
      vars->mpe->nativeCodeCache.X86Emit_MOVIM(imm & 0xFFFFFFF0, x86MemPtr::x86MemPtr_dword, ((uintptr_t)&(vars->mpe->sp)));
      break;
    case (0x500 >> 4):
      //odmactl
    case (0x510 >> 4):
